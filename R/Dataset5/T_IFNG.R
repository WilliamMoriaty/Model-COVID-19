...
critical
gene<-read.table("/project/wangshun/work/R/COVID-19/CTL_IFNG.txt")
...
newdata=rbind(a1, a2)
write.csv(newdata, "T_IFNG_normal.csv")
...
a2 <- as.matrix(Treg@assays$RNA@data[gene$gene,]) #gene$gene: get the rowname of variable gene
newdata=rbind(a1, a2)
write.csv(newdata, "T_IFNG_moderate.csv")
...
a2 <- as.matrix(Treg@assays$RNA@data[gene$gene,]) #gene$gene: get the rowname of variable gene
newdata=rbind(a1, a2)
write.csv(newdata, "T_IFNG_critical.csv")