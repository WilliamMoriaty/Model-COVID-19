...
critical
gene<-read.table("/project/wangshun/work/R/COVID-19/TIM3.txt")
...
newdata=rbind(a1, a2)
write.csv(newdata, "TIM3_normal.csv")
...
a2 <- as.matrix(Treg@assays$RNA@data[gene$gene,]) #gene$gene: get the rowname of variable gene
newdata=rbind(a1, a2)
write.csv(newdata, "TIM3_moderate.csv")
...
a2 <- as.matrix(Treg@assays$RNA@data[gene$gene,]) #gene$gene: get the rowname of variable gene
newdata=rbind(a1, a2)
write.csv(newdata, "TIM3_critical.csv")