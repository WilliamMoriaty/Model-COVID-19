...
critical
gene<-read.table("/project/wangshun/work/R/COVID-19/LAG3.txt")
...
newdata=rbind(a1, a2)
write.csv(newdata, "LAG3_normal.csv")
...
a2 <- as.matrix(Treg@assays$RNA@data[gene$gene,]) #gene$gene: get the rowname of variable gene
newdata=rbind(a1,a2)
write.csv(newdata, "LAG3_moderate.csv")
...
a2 <- as.matrix(Treg@assays$RNA@data[gene$gene,]) #gene$gene: get the rowname of variable gene
newdata=rbind(a1, a2)
write.csv(newdata, "LAG3_critical.csv")