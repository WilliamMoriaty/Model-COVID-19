...
bool CCell::UpdateX_in(double dt)// Update intracelluar virus
{
  ...
  {
    lambda2=_par.lambda2;
    K1=_par.K1;
  }
  yp[0]=y[0];
  yp[1]=y[1]+dt*(_par.k_on*(_R-y[1])*y[0]-_par.k_off*y[1]-_par.k_in*y[1]);
  yp[2]=y[2]+dt*(_par.k_in*y[1]/(_MD.v_cell)+_par.lambda1*V1[0]*_par.b1*pow(K1,_par.m1)/(pow(_par.K1,_par.m1)+pow(y[4],_par.m1))-_par.q0*V2[0]-_par.d1*y[2]);
  yp[3]=y[3]+dt*(lambda2*y[2]+_par.b2*pow(y[3],_par.m2)/(pow(_par.K2,_par.m2)+pow(y[3],_par.m2))-_par.d2*y[3]);
  yp[4]=y[4]+dt*(_par.lambda3*y[3]-_par.d3*y[4]);
  
  yc[0]=y[0];
  yc[1]=y[1]+dt*(_par.k_on*(_R-yp[1])*yp[0]-_par.k_off*yp[1]-_par.k_in*yp[1]);
  yc[2]=y[2]+dt*(_par.k_in*yp[1]/(_MD.v_cell)+_par.lambda1*V1[1]*_par.b1*pow(K1,_par.m1)/(pow(_par.K1,_par.m1)+pow(yp[4],_par.m1))-_par.q0*V2[1]-_par.d1*yp[2]);
  yc[3]=y[3]+dt*(lambda2*yp[2]+_par.b2*pow(yp[3],_par.m2)/(pow(_par.K2,_par.m2)+pow(yp[3],_par.m2))-_par.d2*yp[3]);
...
double CCell::fdeathrate(double T1,float dt)
{
  ...
  if(_MD.sign)
  {
    eta=_par.eta0*T1*dt+_MD.eps_5*dt;
  }
  else
  {
