...
void System::Run()
{
  ...
  step=0;t1=-1; //30 day=720h t1:characterize incubation
  ...
  for(t=0;t<=_MD.T;t=t+_MD.dt)
  {
    ...
    if(SystemUpdate(_MD.dt))
    {
      ...
      {
       if((step*_MD.dt)>_MD.omega1)
       {
        AI.signal=2;// begining of treatment
        _MD.sign=true; //begining of treatment
       }
      }
    }
  }
...
void  System::UpdateX_ex(double dt)// DDE23
{
  ...
  yp[0]=y[0]+dt*(_MD.v_cell*sum1_1*1.0/(_MD.N0*_MD.v_ex)+sum3_1*1.0/(_MD.v_ex*_MD.N0)-sum2_1*1.0/(_MD.v_ex*_MD.N0)-d4*y[0]);
  ...
  yc[0]=y[0]+dt*(_MD.v_cell*sum1_2*1.0/(_MD.N0*_MD.v_ex)+sum3_2*1.0/(_MD.v_ex*_MD.N0)-sum2_2*1.0/(_MD.v_ex*_MD.N0)-d4*yp[0]);
...
double System::NumerialIntegrate(int length)//tau3=7200*dt=72hr,
{
  ...
  { 
    Y[i]=pow(AI.CYT[i],_MD.m4)/(pow(K4,_MD.m4)+pow(AI.CYT[i],_MD.m4));
  }
